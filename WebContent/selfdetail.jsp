...
<head>
<meta charset="utf-8">
<title>${user.urealname }(@${user.uaite })&nbsp;|&nbsp;Poros</title>
...
<script type="text/javascript">
	...
	function baocunxiugai() {
		...
		$.ajax({
			url : '/Poros/user.do?method=xiugaiziliao',
			type : 'POST',
			data : {
				urealname : realname,
				uabout : about,
...
<script>
	...
	function jiaZai() {
		if( $("#back").css("display") == "none"){
			$('#jiazai img').fadeIn();
			var url = '/Poros/tweettwo.do?method=gettweets&num=one&pagenum='+page;
			$.ajax({
				url : url,
				type : 'POST',
				success : function(response, status) {
	...
	function hasNew() {
		$.ajax({
			url : '/Poros/tweettwo.do?method=gettweets&num=one&pagenum='+page,
			type : 'POST',
			success : function(response, status) {
	...
	for (var i = 0; i < tweets.length; i++) {
	 if(tweets[i].tcontent =="" && tweets[i].tpic == null){
				 	...
				 	+porosPath+'/img/'+tweets[i].uname+"/"+tweets[i].ulogo+ '\');background-size: 60px 60px"></div><div class="sender_info"><div class="sender_name" style="font-weight: bold;">'
...
<script>
	...
	function checkaite() {
		...
		xmlHttp.open("GET", "/Poros/user.do?method=checkaite&aite=" + txt, false);
		xmlHttp.send(null);
	}
</script>
</html>